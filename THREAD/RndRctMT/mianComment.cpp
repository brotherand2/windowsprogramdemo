...
typedef DWORD(*PFNDEMO)();
void MyThread(PVOID pvoid)
{
	ClientSize *size = (ClientSize*)pvoid;
	int red, green, blue, left, right, top, bottom;
	srand(GetTickCount());
	...
	while (true)
	{
		if (size->cxClient&&size->cyClient)
		{
			left = rand() % (size->cxClient);
			right = rand() % (size->cxClient);
			top = rand() % (size->cyClient);
			bottom = rand() % (size->cyClient);
			red = rand() % 256;
			green = rand() % 256;
			blue = rand() % 256;
...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	case WM_CREATE://窗口被创建
		...
		size.hwnd = hwnd;
		_beginthread(MyThread,0,&size);
		break;
	case WM_SIZE://窗口大小改变时
		size.cxClient = LOWORD(lParam);//更新客户区的大小
		size.cyClient = HIWORD(lParam);
		break;
