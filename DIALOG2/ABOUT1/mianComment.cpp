...
long _stdcall ElliPushProc(HWND hwnd, unsigned message, unsigned wParam, long lParam)
{
	...
	case WM_PAINT:
		...
		GetWindowText(hwnd, szCaption, 64);//得到窗口标题
		Ellipse(hdc, rect.left, rect.top, rect.right, rect.bottom);//在客户区画一个椭圆
		DrawText(hdc, szCaption, -1, &rect,DT_SINGLELINE|DT_CENTER|DT_VCENTER);//在客户区中间显示标题
		EndPaint(hwnd, &ps);
		return 0;
	case WM_KEYDOWN:
		if (wParam != VK_SPACE)
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, char *  szCmdCommand, int nCmdShow)
{
	...
	wndclass.hIcon = LoadIcon(hInstance,TEXT("ABOUT"));//图标
	wndclass.hInstance = hInstance;//实例名柄，程序的PID，一个整数，根据这个整数，系统可以找到进程的地址
	wndclass.lpfnWndProc = WndProc;//窗口过程
	wndclass.lpszClassName = TEXT("zeng");//类名
	wndclass.lpszMenuName = TEXT("ABOUT");//菜单
