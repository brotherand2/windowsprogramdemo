...
bool MyReadFile(HWND hEdit,HWND hStatus, TCHAR szFileName[])//读取文件内容
{
 ...
 HANDLE hTemplateFile    // 用于复制文件句柄 
	...
	*/
	...
	{
	case ANSI://ansi编码文件
		//MessageBox(nullptr, TEXT("ansi txt"), TEXT(""), 0);
...
bool MyFindText(HWND hEdit,FINDREPLACE &fr,bool bDown)//查找文本
{
	...
	int pos ;//查找到的位置
	int start;//当前选中文字的开始位置，若未选中则是当前光标相对于开头位置的偏移（字符个数）
	int end;//当前选中文字的结束位置+1，若未选中则是当前光标相对于开头位置的偏移（字符个数）
	SendMessage(hEdit, EM_GETSEL, (unsigned)&start,(long)&end);//当前选中文字的开始位置、结束，若未选中则是当前光标相对于开头位置的偏移（字符个数）
	if (start != end&&bDown)//开始结束位置不同，说明选中文字并且是向下查找
		start++;//则从当前选中位置的下一个位置继续查找,否则没选中则从当前位置开始查找
	...
	if (bDown)//向下查找
	{
		auto p=search(pText+start,pText+len, fr.lpstrFindWhat , fr.lpstrFindWhat+findLen);//查找第一个匹配的字符串位置
	...
	{
		auto p = find_end(pText,pText + start, fr.lpstrFindWhat, fr.lpstrFindWhat + findLen);//查找从最后一个匹配的字符串位置
		if (p == pText + start)//向上查找不到
			return false;
		pos = p - pText;
	}
...
void InitFind(HWND hEdit,TCHAR szFindText[])//初始化查找对话框
{
	int start;//当前选中文字的开始位置，若未选中则是当前光标相对于开头位置的偏移（字符个数）
	int end;//当前选中文字的结束位置+1，若未选中则是当前光标相对于开头位置的偏移（字符个数）
	SendMessage(hEdit, EM_GETSEL, (unsigned)&start, (long)&end);//当前选中文字的开始、结束位置，若未选中则是当前光标相对于开头位置的偏移（字符个数）			
	if (start != end)//如果开始和结束位置不相等说明选中了文字
	{
		int len = GetWindowTextLength(hEdit);//获得编辑框字符个数
		TCHAR *pText = new TCHAR[len + 1];//分配空间
		shared_ptr<TCHAR>sp1(pText, [](TCHAR *p){delete[]p; });//将分配的空间交智能指针管理	
		GetWindowText(hEdit, pText, len + 1);//获得编辑框字符
		tstring temp(pText + start, pText + end);//获得选中的字符串
...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	static tstring  statusInfo[5]{TEXT("新建文件"),TEXT(""),TEXT(""),TEXT(""),TEXT("")};//状态栏5条信息
	int xPos[5]{0, 0, 0,0};//状态栏的四个右横坐标
	int row, col;//光标的行列
	switch (UMsg)
	{
	case WM_CREATE://窗口被创建
		...
		hStatus = CreateWindow(STATUSCLASSNAME, 0, WS_CHILD | WS_VISIBLE,
			0, 0,0,0, hwnd, (HMENU)IDS_STATE, hInst, nullptr);
		for (int i = 0; i <5; i++)
	...
	case WM_SIZE://窗口大小改变时
		...
		SendMessage(hStatus, SB_SETPARTS,5, (long)xPos);//根据四个横坐标成四栏
		...
		MoveWindow(hStatus, 0,0, 0,0, true);//调整状态栏在客户区最下面，不用设置大小，因为大小固定不可变
		return 0;
	...
	case WM_COMMAND://在子窗口、菜单上操作时产生命令
		...
		case IDM_EDIT_DELETE://删除选中   未处理删除后面
			...
			break;
		case IDM_FORMAT_AUTOLINE://自动换行    功能失败
			...
			MyReadFile(hEdit,hStatus,szFileName);//重新读取文件内容
			return 0;
	...
	case WM_TIMER://每一秒更新一次时间
	{
		...
		return 0;
	case WM_LBUTTONDOWN://当鼠标左键按下的时候
		row = SendMessage(hEdit, EM_LINEFROMCHAR, -1, 0);
		statusInfo[2]=TEXT("行号:")+toString(row);//将行号转换成字符串
		SendMessage(hStatus, SB_SETTEXT,2, (long)statusInfo[2].c_str());//对第二栏更新时间 
		MessageBox(hwnd, _T("aaa"),0,0);
		return 0;
	...
	default:
		if (UMsg == WM_FINDREPLACE)//不能放在case 语句中，因为case语句要常量，而WM_FINDREPALCE是自己定义的一个变量
		{
			...
			if (pfr->Flags&FR_REPLACE)//如果按下了替换下一个
			{
				...
				int end;//当前选中文字的结束位置+1，若未选中则是当前光标相对于开头位置的偏移（字符个数）
				SendMessage(hEdit, EM_GETSEL, (unsigned)&start, (long)&end);//当前选中文字的开始、结束位置，若未选中则是当前光标相对于开头位置的偏移（字符个数）			
				if (start != end)//如果开始和结束位置不相等说明选中了文字，进行替换
