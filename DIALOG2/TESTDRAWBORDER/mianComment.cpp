...
void DrawBorder(HWND hwnd)
{
	...
	GetWindowRect(hwnd, &rect);//获得窗口矩形的屏幕坐标 
	ClientToScreen(hwnd, &pt);//将客户区坐标转换成屏幕坐标

	bx = pt.x - rect.left;//窗口与客户区横坐标、纵坐标的距离
	by = pt.y - rect.top;
	GetClientRect(hwnd, &rect);
	rect.bottom -=26;
	OffsetRect(&rect, bx, by);//将客户区坐标转换成窗口坐标
	hRgn2 = CreateRectRgnIndirect(&rect);
	InflateRect(&rect, -1, -1);//横坐标、纵坐标增加2个像素
...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	case WM_CREATE://窗口被创建
		...
		hEdit = CreateWindow(TEXT("edit"), TEXT(""), WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_AUTOHSCROLL | ES_MULTILINE | ES_NOHIDESEL, 0, 0, 0, 0,
			hwnd, (HMENU)1, hInst, nullptr);//创建有边界的多行编辑框，ES_NOHIDESEL在编辑框失去焦点后选中的文本依然保持高亮显示
	...
	case WM_SIZE://窗口大小改变时
		...
		MoveWindow(hEdit, 2, 2, cxClient-4, cyClient-29, true);//调整编辑大小为整个客户区
