...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	HINSTANCE hInst;
	static HMENU hMenuMain, hMenuFile, hMenuEdit;//主菜单、文件菜单、编辑菜单
	switch (UMsg)
	{
	case WM_CREATE://窗口被创建
		...
		hMenuMain = LoadMenu(hInst, TEXT("MENUMAIN"));//读取主菜单
		hMenuFile = LoadMenu(hInst, TEXT("MENUFILE"));//读取文件菜单
		hMenuEdit = LoadMenu(hInst, TEXT("MENUEDIT"));//读取编辑菜单
		break;
	...
	case WM_COMMAND://在子窗口、菜单上操作时产生命令
		...
		{
		case IDM_FILE:
			SetMenu(hwnd, hMenuFile);//设置文件菜单
			break;
		case IDM_EDIT:
			SetMenu(hwnd, hMenuEdit);//设置编辑菜单
			break;
		case IDM_MAIN:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, char *  szCmdCommand, int nCmdShow)
{
	...
	wndclass.lpszMenuName = TEXT("MENUMAIN");//菜单
