...
#endif
static double cxClient, cyClient;
...
void OffsetPoint(HDC hdc, int num, int colon)
{
	for (int i = 0; i < 7; i++)
	{
		for (int j = 0; j < 6; j++)
		{
			copyPoint[i][j].x =(ptSegment[i][j].x + num * 42 + colon * 12)*flate;
			copyPoint[i][j].y = (ptSegment[i][j].y)*flate;
		}
	}
}
void OffsetColon(HDC hdc, int num, int col)
{
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			copyColon[i][j].x = (colon[i][j].x + num * 42 + col * 12)*flate;
			copyColon[i][j].y = colon[i][j].y*flate;
		}
	}
}
...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)
{
	...
	case WM_SIZE:
		...
		flate = min(cxClient / 276, cyClient / 72);
		break;
	case WM_PAINT:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, char *  szCmdCommand, int nCmdShow)
{
	...
	HWND hwnd = CreateWindowEx(WS_EX_TOPMOST,TEXT("zeng"), 0, WS_POPUP,
		1212, 102, 100, 30,
