...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	case WM_CREATE://窗口被创建
		...
		hdcHemf = CreateEnhMetaFile(0,TEXT("emf4.emf"), 0,TEXT("EMF4\0EMF Demo #4\0"));
		if (!hdcHemf)
			return 0;
		hbm = LoadBitmap(0, MAKEINTRESOURCE(OBM_CLOSE));
		GetObject(hbm, sizeof(BITMAP), &bm);
		hdcMem = CreateCompatibleDC(hdcHemf);
		SelectObject(hdcMem, hbm);
		StretchBlt(hdcHemf, 100, 100, 100, 100, hdcMem, 0, 0, bm.bmWidth, bm.bmHeight, SRCCOPY);
		DeleteDC(hdcMem);
		DeleteObject(hbm);
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		if (hemf=GetEnhMetaFile(TEXT("emf4.emf")))
		{
