...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	case WM_CREATE://窗口被创建
		...
		hdcHemf = CreateEnhMetaFile(0,TEXT("emf3.emf"), 0,TEXT("EMF3\0EMF DEMO3 #2\0"));
		if (!hdcHemf)
			return 0;
		SelectObject(hdcHemf, CreateSolidBrush(RGB(0, 0, 255)));
		lb.lbColor = RGB(255, 0, 0);
		lb.lbHatch = 0;
		lb.lbStyle = BS_SOLID;
		SelectObject(hdcHemf, ExtCreatePen(PS_SOLID | PS_GEOMETRIC, 5, &lb, 0, 0));
		Rectangle(hdcHemf, 100, 100, 201, 201);
		MoveToEx(hdcHemf, 100, 100, 0);
		LineTo(hdcHemf, 200, 200);
		MoveToEx(hdcHemf, 100, 200, 0);
		LineTo(hdcHemf, 200, 100);
		DeleteObject(SelectObject(hdcHemf, GetStockObject(BLACK_PEN)));
		DeleteObject(SelectObject(hdcHemf, GetStockObject(WHITE_BRUSH)));
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		if (hemf=GetEnhMetaFile(TEXT("emf3.emf")))
		{
