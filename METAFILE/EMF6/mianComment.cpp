...
int _stdcall EnhMetaFileProc(HDC hdc,HANDLETABLE *hmf, const ENHMETARECORD *pEmfRecord, int iHandles, long pDdata)
{
	...
	PENHMETAHEADER pEmfCopy;
	pEmfCopy = (PENHMETAHEADER)malloc(pEmfRecord->nSize);
	CopyMemory(pEmfCopy, pEmfRecord, pEmfRecord->nSize);
	if (pEmfRecord->iType == EMR_RECTANGLE)
	{
		//PlayEnhMetaFileRecord(hdc, hmf, (const ENHMETARECORD*)pEmfCopy, iHandles);
		pEmfCopy->iType = EMR_ELLIPSE;
	}
	PlayEnhMetaFileRecord(hdc, hmf, (const ENHMETARECORD*)pEmfCopy, iHandles);
	free(pEmfCopy);
	return true;
...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		if (hmf = GetEnhMetaFile(TEXT("emf3.emf")))
		{
			EnumEnhMetaFile(hdc, hmf, EnhMetaFileProc, 0, &rect);
			DeleteEnhMetaFile(hmf);
		}
		else
			MessageBox(hwnd, TEXT("emf3.bmf 不存在"), 0, 0);
