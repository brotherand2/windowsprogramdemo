...
#endif
int _stdcall EnhMetaFileProc(HDC hdc,HANDLETABLE *hmf, const ENHMETARECORD *pEmfRecord, int iHandles, long pDdata)
{
	LOGBRUSH lb;
	HBRUSH hBrush;
	HPEN hPen;
	if (pEmfRecord->iType != EMR_HEADER&&pEmfRecord->iType != EMR_EOF)
		PlayEnhMetaFileRecord(hdc, hmf, pEmfRecord, iHandles);
	if (pEmfRecord->iType == EMR_RECTANGLE)
	{
		...
		hPen=(HPEN) SelectObject(hdc, ExtCreatePen(PS_SOLID | PS_GEOMETRIC, 5, &lb, 0, 0));
...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	case WM_CREATE://窗口被创建
		...
		if (hOldEmf = GetEnhMetaFile(TEXT("emf3.emf")))
			GetEnhMetaFileHeader(hOldEmf, sizeof(ENHMETAHEADER), &emh);
		else
		{
			MessageBox(hwnd, TEXT("emf3.emf 不存在"), 0, 0);
			return 0;
		}
		hdcEmf = CreateEnhMetaFile(0, TEXT("emf7.emf"), 0, TEXT("EMF7\0EMF Demo#7\0"));
		EnumEnhMetaFile(hdcEmf, hOldEmf, EnhMetaFileProc, 0, (RECT*)&emh.rclBounds);
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		hmf = GetEnhMetaFile(TEXT("emf7.emf"));
		if (hmf)
		{
