...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//主窗口 窗口过程
{
	...
	static int nSelectedLine;//当前被选中的行
	...
	case WM_LBUTTONDOWN://鼠标单击
		...
		{
			nSelectWallpaper=nSelectedLine +iVertPos;//nSelectedLine加上当前垂直滚动条的位置，即当前的选中的壁纸
	...
	case WM_VSCROLL://垂直滚动
		...
		if (iVertPos != si.nPos)//滚动条位置变化了
			...
			InvalidateRect(hwnd, &scrollRect, true);
		break;
	case WM_HSCROLL:
		si.cbSize = sizeof(si);
		si.fMask = SIF_ALL;//获得滚动条所有信息
	...
	case WM_PAINT:
		...
		if (fileName.size() > 0)
		{
			...
			iHorzPos = si.nPos;
			start = max(0, min(fileName.size(), iVertPos + ps.rcPaint.top/cyChar));//滚动条滚动后，ps.rcPaint.top/cyChar要重绘的起始行，加上iVertPos获得在滚动条上绘制的位置，最小为1是因为要不包括第1行的列名
			...
			for (int i = start,j=0; i <= end; i++,j++)//对滚动操作后的失效区域重绘
			{
				...
				int y = i - iVertPos+1;//垂直绘制的起始位置
				GetFileAttributesEx((wallpaper + fileName[i]).c_str(),GetFileExInfoStandard,&attri);//得到当前文件的属性,GetFileExInfoStandard表示表准文件属性，最后一个参数是 WIN32_FILE_ATTRIBUTE_DATA结构的指针,i-1是因为下标是个数-1，如第3个，下标为2
				...
				StringCchPrintf(szBuffer,1024, TEXT("%3d%-5c%-15s%-20s%-15s%-30s%-30s%-30s%-30s"), i,':',fileName[i].c_str(),fileSize.c_str(),
					...
					filePath.c_str());//格式化字符串，i-1是因为下标是个数-1，如第3个，下标为2
