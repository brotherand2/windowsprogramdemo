...
void writeToXML(HWND hwnd)
{
	...
	myDocument.LinkEndChild(new TiXmlDeclaration("1.0", "utf-8",""));//添加声明
	...
	addTextToNewElement(element, "运行最小化托盘显示", runningMiniShow);
	addTextToNewElement(element, "气泡功能关闭", closeBubble);
	RootElement->LinkEndChild(element);
	...
	element->SetIntAttribute("x", windowSize.left);
	element->SetIntAttribute("y", windowSize.top);
	element->SetIntAttribute("width", windowSize.right);
	element->SetIntAttribute("heigh", windowSize.bottom);
	RootElement->LinkEndChild(element);
	...
	element = new TiXmlElement("图片大小不得低于多少KB");//图片大小不得低于(KB):
	element->SetIntAttribute("size", miniPhotoSize);
	RootElement->LinkEndChild(element);
	...
	RootElement->LinkEndChild(element);
	element = new TiXmlElement("被过滤的窗口名");//被过滤的窗口名
	saveDirectory(element, "窗口", filterWindowName);
	RootElement->LinkEndChild(element);
...
bool readFromXML(HWND hwnd)
{
	...
	if(element)
	{
		...
		GetTextFromBool(element, runningMiniShow, "运行最小化托盘显示");
		GetTextFromBool(element, closeBubble, "气泡功能关闭");
	}
	...
	if(element)
	{
		...
		windowSize.left = makeInRange(0, temp, cxScreen);
		element->Attribute("y", &temp);
		windowSize.top = makeInRange(0, temp, cyScreen);
		element->Attribute("width", &temp);
		windowSize.right = makeInRange(0, temp, cxScreen);
		element->Attribute("heigh", &temp);
		windowSize.bottom = makeInRange(0, temp, cyScreen);
	}
	...
	//读取图片最小的大小
	element = rootElement->FirstChildElement("图片大小不得低于多少KB");
	if (element)
	{
		int temp;
		element->Attribute("size", &temp);
		miniPhotoSize = makeInRange(0, temp, 10240);
	}
	//读取字体信息
	...
	element = rootElement->FirstChildElement("被过滤的窗口名");
	readDirectory(element, filterWindowName);
	return true;
}
//创建弹出菜单
...
void FindFile(tstring CurrentPath,tstring type)//查找文件
{
	...
	if (handle != INVALID_HANDLE_VALUE)//找到了
	{
		do
		{
			if(!isLessThanMiniSize(fd.nFileSizeHigh,fd.nFileSizeLow,miniPhotoSize))
...
int _stdcall EnumWindowProc(HWND hwnd, long lParam)//枚举函数的窗口过程
{
	...
	{//如果存在这个窗口，并且窗口可见，不是最小化
		...
		{
			auto result = find(filterWindowName.begin(), filterWindowName.end(), title);
			if(result==filterWindowName.end())//如果当前窗口名未被列入过滤名单，保存
...
int _stdcall DlgGotoProc(HWND hDlg, unsigned message, unsigned wParam, long lParam)//转到对话框窗口过程
{
	...
	case WM_CTLCOLORDLG:
	{
		HBRUSH hBrush = CreatePatternBrush(LoadBitmap((HINSTANCE)GetWindowLong(hDlg, GWL_HINSTANCE), MAKEINTRESOURCE(IDB_BITMAP10)));
...
int _stdcall DlgWallpaperSet(HWND hDlg, unsigned int message, unsigned wParam, long lParam)
{
	...
	int count;//列表框内容个数
	int index;//列表框上索引
	int originMiniPhotoSize;//原图片最小大小
	switch (message)
	{
	case WM_INITDIALOG://对话框初始化
		...
		SetDlgItemInt(hDlg, IDC_MINISIZE, miniPhotoSize, false);
		...
		SendMessage(randomChange?hRadioRandom:hRadioSequence, BM_SETCHECK, 1, 0);//如果是随机换
		SendMessage(hCheckBoxAutoBoot,BM_SETCHECK,startingUpBoot, 0);
		SendMessage(GetDlgItem(hDlg, IDC_STARTINGMINISHOW), BM_SETCHECK, runningMiniShow, 0);
		SendMessage(GetDlgItem(hDlg, IDC_SHUTDOWNBIBLE), BM_SETCHECK, closeBubble, 0);
		...
		for (int i = 0; i < allTitle.size(); i++)//显示过滤的窗口名
			SendMessage(hListCurrentWindow, LB_INSERTSTRING, -1, (long)allTitle[i].c_str());
		break;
	case WM_CTLCOLORDLG:
	{
		HBRUSH hBrush = CreatePatternBrush(LoadBitmap((HINSTANCE)GetWindowLong(hDlg, GWL_HINSTANCE), MAKEINTRESOURCE(IDB_BITMAP11)));
		return (long)hBrush;//这里不能用break,否则会调用下面的DefWindowProc，将颜色设回默认的，返回一个画刷，编辑框的默认窗口过程会用这个画刷作为背景色
	};
	case WM_COMMAND:
		switch (wmID)
		...
		case  IDC_EDITCONTENT:
			...
			break;
		case IDC_BTNFILTERWINDOW:
			index = SendMessage(hListCurrentWindow, LB_GETCURSEL, 0, 0);//获得当前选中目录的索引
			if (index != LB_ERR)
			{
				SendMessage(hListCurrentWindow, LB_GETTEXT, index, (long)szBuffer);//获得当前选中的路径
				if (lstrcmp(szBuffer, TEXT("Program Manager")) == 0)
					Message(TEXT("桌面的窗口名就是Program Manager，不能被过滤"));
				else
				{
					SendMessage(hListFilterWindow, LB_INSERTSTRING, -1, (long)szBuffer);//将当前目录插入到左列表框尾部
					SendMessage(hListCurrentWindow, LB_DELETESTRING, index, 0);//从右列表框中删除
				}
			}
			break;
		case IDM_SETFINISH://设置完成
			...
			if (!closeBubble)
			{
				InitNotifyIconData(hwnd, hInst, nid, IDI_ICON1,closeBubble);//初始化托盘图标结构
			...
			originMiniPhotoSize = miniPhotoSize;
			miniPhotoSize = GetDlgItemInt(hDlg, IDC_MINISIZE, 0, false);
			if (originMiniPhotoSize != miniPhotoSize)
				FindPhoto();//重新查找图片
			writeToXML(hwnd);
...
int _stdcall DlgModifyDirectoryProc(HWND hDlg, unsigned int message, unsigned wParam, long lParam)
{
	...
	case WM_COMMAND:
		...
		{
			int len = lstrlen(szDirectoryPath);
			if (szDirectoryPath[len - 1] == TEXT('\\'))
				szDirectoryPath[len - 1] = TEXT('\0');//如果是盘符如d:\ e:\去掉/
			count = SendMessage(hListRight, LB_GETCOUNT, 0, 0);//得到右列表框中目录的个数
			bool exist = false;
			TCHAR path[MAX_PATH];
			for (int i = 0; i < count; i++)
			{
				SendMessage(hListRight, LB_GETTEXT, i, (long)path);//获得当前索引的路径
				if(lstrcmp(path,szDirectoryPath)==0)
				{
					exist = true;
					Message(TEXT("已经存在此路径"));
				}
			}
			if(!exist)
				SendMessage(hListRight, LB_INSERTSTRING, -1, (long)szDirectoryPath);//将当前目录插入到右列表框尾部
			SetWindowText(hEditPath, TEXT(""));
		}
			break;
...
void InitWindow(HWND hwnd,HINSTANCE hInst)//用读取到的数据初始化
{
	...
	if (windowSize.right > 0 && windowSize.bottom > 0 && windowSize.left >= 0 && windowSize.top >= 0)//如果读取的宽度和高度不为0
		MoveWindow(hwnd, windowSize.left, windowSize.top, windowSize.right, windowSize.bottom, true);//移动窗口，显示上一次窗口启动的位置大小
}
...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//主窗口 窗口过程
{
	...
	case WM_COMMAND:
	{
		...
		case IDM_EDIT_GOTO: //转到
			...
			return 0;
		case IDM_FILEBAT://文件批处理
			DialogBox(hInst, TEXT("IDD_FILEBAT"), hwnd, DlgFileBatProc);
			break;
