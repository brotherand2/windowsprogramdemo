...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		dtParam.cbSize = sizeof(DRAWTEXTPARAMS);
		dtParam.iLeftMargin = 10;
		dtParam.iRightMargin = 20;
		dtParam.iTabLength = 10;
		dtParam.uiLengthDrawn = 0;
		SetRect(&rect, 10, 10, 310, 210);
		Rectangle(hdc, rect.left, rect.top, rect.right, rect.bottom);
		//DrawText(hdc, p2, -1, &rect,DT_VCENTER|DT_SINGLELINE|DT_CENTER);
		DrawTextEx(hdc, pt, -1, &rect, DT_WORDBREAK|DT_EXPANDTABS|DT_TABSTOP,&dtParam);
