...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	int tab[] = { 100, 200, 300, 500 };
	TCHAR szBuffer[128] = TEXT("Hello\tWorld\tAgain\tAnd\tAgain\t");
	int tabstop =200;
	switch (UMsg)
	{
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		for (int i = 0; i < 15; i++)
		{
			MoveToEx(hdc, i * 100, 50, 0);
			LineTo(hdc, i * 100, 80);
			for (int j = 0; j < 10; j++)
			{
				MoveToEx(hdc, i * 100 + j * 10, 50, 0);
				LineTo(hdc, i * 100 + j * 10, 70);
			}
		}
		TabbedTextOut(hdc, 10, 100, szBuffer, -1,4,tab,0);
		TabbedTextOut(hdc, 10, 120, szBuffer, -1, 4, tab, 50);
		TabbedTextOut(hdc, 10, 160, szBuffer, -1, 1, &tabstop, 0);
		TabbedTextOut(hdc, 10, 180, szBuffer, -1, 1, &tabstop, 50);
		TabbedTextOut(hdc, 10, 210, szBuffer, -1,0,0, 0);
		TabbedTextOut(hdc, 10, 230, szBuffer, -1,0,0, 50);
