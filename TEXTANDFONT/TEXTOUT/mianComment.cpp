...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	RECT rect;
	TCHAR szBuffer[128]=TEXT("Hello World");
	switch (UMsg)
	{
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		SetRect(&rect, 100, 100, 600, 300);
		FrameRect(hdc, &rect, CreateSolidBrush(RGB(255, 0, 0)));
		MoveToEx(hdc, 100, 200, 0);
		LineTo(hdc, 600, 200);
		TextOut(hdc, 100, 100, szBuffer, lstrlen(szBuffer));
		SetTextAlign(hdc, TA_RIGHT);
		TextOut(hdc, 600, 100, szBuffer, lstrlen(szBuffer));
		SetTextAlign(hdc, TA_CENTER);

		TextOut(hdc, 350, 100, szBuffer, lstrlen(szBuffer));
		SetTextAlign(hdc, TA_BOTTOM|TA_LEFT);
		TextOut(hdc, 100, 300, szBuffer, lstrlen(szBuffer));
		SetTextAlign(hdc, TA_BOTTOM|TA_CENTER);
		TextOut(hdc, 350, 300, szBuffer, lstrlen(szBuffer));
		SetTextAlign(hdc, TA_BOTTOM|TA_RIGHT);
		TextOut(hdc, 600, 300, szBuffer, lstrlen(szBuffer));
		SetTextAlign(hdc, TA_BASELINE|TA_LEFT);
		TextOut(hdc, 100, 200, szBuffer, lstrlen(szBuffer));
		SetTextAlign(hdc, TA_BOTTOM|TA_CENTER);
		TextOut(hdc, 350, 200, szBuffer, lstrlen(szBuffer));
		SetTextAlign(hdc, TA_BASELINE|TA_RIGHT);
		TextOut(hdc, 600, 200, szBuffer, lstrlen(szBuffer));
