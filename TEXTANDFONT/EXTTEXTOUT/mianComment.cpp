...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		ExtTextOut(hdc, 100, 30, 0, 0, TEXT("ABCDEF"), 6, 0);
		ExtTextOut(hdc, 100, 50, 0, 0, TEXT("ABCDEF"), 6, dis);
		SetBkColor(hdc, RGB(255, 0, 0));
		ExtTextOut(hdc, 100, 70,ETO_OPAQUE, 0, TEXT("ABCDEF"), 6, 0);
		SetBkColor(hdc, RGB(255, 255, 255));
		rect.left = 100;
		rect.top = 100;
		rect.right = 130;
		rect.bottom = 110;
		ExtTextOut(hdc, 90, 100, ETO_CLIPPED, &rect, TEXT("ABCDEF"), 6, 0);
		SetBkColor(hdc, RGB(255,0,0));
		rect.left = 100;
		rect.top = 130;
		rect.right = 135;
		rect.bottom = 150;
		ExtTextOut(hdc, 100, 130, ETO_CLIPPED|ETO_OPAQUE, &rect, TEXT("ABCDEF"), 6, dis);
