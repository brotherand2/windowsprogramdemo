...
void ShutDown()
{
	...
	if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES, &hToken))//获得当前进程的修改权限的访问令牌
		return;
	LookupPrivilegeValue(0, SE_SHUTDOWN_NAME, &tkp.Privileges[0].Luid);//获得本机关机的标识
	tkp.PrivilegeCount = 1;
	tkp.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;//开启关机特权
	AdjustTokenPrivileges(hToken, false, &tkp, sizeof(tkp), 0, 0);//修改特权
	if (GetLastError() != ERROR_SUCCESS)
		return;
	InitiateSystemShutdown(0, TEXT("\n哈哈\n....\n傻逼开始开机了"), 30, false, 0);
}
