...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		Rectangle(hdcMask, 0, 0, cxSource, cySource);
		SelectObject(hdcMask, GetStockObject(WHITE_BRUSH));
		Ellipse(hdcMask, 12, 12, cxSource-12, cySource-12);
		cxLocation = (cxClient - cxSource) / 2;
		cyLocation = (cyClient - cySource) / 2;
		BitBlt(hdcMem, 0, 0, cxSource, cySource, hdcMask, 0, 0, SRCAND);
		BitBlt(hdc, cxLocation, cyLocation, cxSource, cySource, hdcMask, 0, 0, 0x220326);//~s&d
		BitBlt(hdc,cxLocation,cyLocation, cxSource, cySource, hdcMem, 0, 0, SRCPAINT);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, char *  szCmdCommand, int nCmdShow)
{
	...
	wndclass.hbrBackground = (HBRUSH)GetStockObject(LTGRAY_BRUSH);//背景画刷
