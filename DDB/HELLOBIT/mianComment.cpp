...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	case WM_COMMAND://在子窗口、菜单上操作时产生命令
		...
		case IDM_BIG:
		case IDM_SMALL:
			CheckMenuItem(hMenup, check, MF_UNCHECKED);
			check = wmId;
			CheckMenuItem(hMenup, wmId, MF_CHECKED);
			break;
		}
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		{
	case IDM_BIG:
		StretchBlt(hdc, 0, 0, cxClient, cyClient, hdcMem, 0, 0, size.cx, size.cy, SRCCOPY);
		break;
	case IDM_SMALL:
		for (int i = 0; i < cxClient; i += size.cx)
		{
			for (int j = 0; j < cyClient; j += size.cy)
				BitBlt(hdc, i, j, size.cx, size.cy, hdcMem, 0, 0, SRCCOPY);
		}
		break;
		}
