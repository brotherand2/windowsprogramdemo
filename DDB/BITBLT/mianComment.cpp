...
long _stdcall WndProc(HWND hwnd, unsigned int UMsg, unsigned int wParam, long lParam)//窗口过程
{
	...
	static int cxSource, cySource;// 以像素计算的标题栏按钮的尺寸
	switch (UMsg)
	{
	case WM_CREATE://窗口被创建
		...
		cxSource = GetSystemMetrics(SM_CXSIZE)+GetSystemMetrics(SM_CXSIZEFRAME);//		以像素计算的标题栏按钮的尺寸
		cySource = GetSystemMetrics(SM_CYSIZE)+GetSystemMetrics(SM_CYSIZEFRAME);
		break;
	...
	case WM_PAINT://有区域失效，发生重绘
		...
		for (int i = 0; i < cxClient; i += cxSource)
			for (int j = 0; j < cyClient; j += cySource)
				BitBlt(hdc, i, j, cxSource, cySource, hdcWindow, 0, 0, SRCCOPY);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, char *  szCmdCommand, int nCmdShow)
{
	...
	wndclass.hIcon = LoadIcon(nullptr,IDI_ERROR);//图标
